...
WORKDIR /workspaces/examplepackage/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/examplepackage/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/examplepackage/ && touch src/examplepackage/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/examplepackage/
